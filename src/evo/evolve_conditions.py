...
def cut_condi_population(population):
    basic_cut_place = int(len(population) * (1.0-EC['DROP_RATIO']))
    ...
    logging.info("Condi_evo: cutting_population to " + str(cut_in) + " elems")
...
def mutate_condi_genome(genome):
    ...
    if random.random() >= EC['MUTATION_RATE']:
...
def evolve_condi_population(population):
    ...
    while amount_of_offspring > 0:                # EVOLVE
        ...
        offspring1 = [uuid.uuid4().hex, mutate_condi_sample(cohab1[1][:pivot] + cohab2[1][pivot:]), 0.0, 0]  # MUTATE OVER CROSSOVER
        offspring2 = [uuid.uuid4().hex, mutate_condi_sample(cohab2[1][:pivot] + cohab1[1][pivot:]), 0.0, 0]  # MUTATE OVER CROSSOVER

        logging.debug("from cohab1 " + str(cohab1))
        logging.debug("and cohab2 " + str(cohab2))
        logging.debug("pivoted at " + str(pivot))
        logging.debug("constructed " + str(offspring1))
        logging.debug("and " + str(offspring2))
...
def main(ns=1, filename=None):
    ...
    population = []
    if filename:
        filename = os.path.abspath(os.path.realpath(os.path.join(os.path.dirname(os.path.realpath('__file__')), filename)))
...
if __name__ == '__main__':
    main(3, "../../logs/v2.0/2019-5-19_211619.population")
